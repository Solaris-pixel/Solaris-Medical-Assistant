...
views.forEach(v=>{
  ...
  mb.textContent = navLabels[v];
  mb.id = 'navm-'+v;
  mb.onclick = ()=>{ showView(v); closeMobileNav(); };
  navMobileEl.appendChild(mb);
});
function showView(name){
  views.forEach(v=>{
    document.getElementById('view-'+v).classList.toggle('active', v===name);
    document.getElementById('nav-'+v).classList.toggle('active', v===name);
    document.getElementById('navm-'+v).classList.toggle('active', v===name);
  });
...
// General educational info only — not a prescription or diagnosis.
const symptomInfo = {
  'Headache': {
    desc: 'Often tension, dehydration, screen strain, or poor sleep. Less commonly linked to migraine or sinus pressure.',
    care: 'Rest in a dim, quiet room, drink water, and consider paracetamol or ibuprofen at standard adult OTC doses.',
    seek: 'Sudden, severe ("worst ever") headache, or one with vision changes, confusion, or stiff neck needs urgent care.'
  },
  'Fever': {
    desc: 'Usually a sign the body is fighting an infection — viral or bacterial.',
    care: 'Rest, fluids, light clothing, and paracetamol or ibuprofen to ease discomfort if needed.',
    seek: 'Fever above 39.5°C (103°F), lasting more than 3 days, or with a rash, stiff neck, or confusion needs medical attention.'
  },
  'Cough': {
    desc: 'Commonly from a cold, irritation, or postnasal drip; can also signal a chest infection or asthma.',
    care: 'Warm fluids, honey (if over age 1), and a soothing lozenge. A short-term OTC cough suppressant can help at night.',
    seek: 'Coughing up blood, lasting over 3 weeks, or with breathlessness or chest pain needs clinician review.'
  },
  'Sore throat': {
    desc: 'Most often viral, sometimes bacterial (e.g. strep) or from dryness/irritation.',
    care: 'Warm salt-water gargles, throat lozenges, and paracetamol or ibuprofen for pain.',
    seek: 'Severe pain, trouble swallowing or breathing, or symptoms beyond a week may need an antibiotic check.'
  },
  'Fatigue': {
    desc: 'Frequently tied to poor sleep, stress, or a recent illness; can also reflect anemia or thyroid issues if persistent.',
    care: 'Prioritize sleep, light activity, balanced meals, and hydration. A short-term break from screens can help too.',
    seek: 'Fatigue lasting more than 2 weeks, or with weight change or breathlessness, deserves a clinician visit.'
  },
  'Nausea': {
    desc: 'Can stem from a stomach bug, food, motion, medication, or stress.',
    care: 'Small sips of clear fluids, bland foods (toast, rice), and ginger tea. An OTC antiemetic can help if needed.',
    seek: 'Nausea with severe abdominal pain, persistent vomiting, or signs of dehydration needs prompt care.'
  },
  'Shortness of breath': {
    desc: 'Can range from anxiety or exertion to asthma, infection, or a heart/lung problem.',
    care: 'Sit upright, try slow controlled breathing. Use a prescribed inhaler if you have one for asthma/COPD.',
    seek: 'New, worsening, or severe breathlessness — especially with chest pain or blue lips — is a medical emergency. Call emergency services.'
  },
  'Chest pain': {
    desc: 'Has many causes from muscle strain to heartburn, but it can also signal a heart or lung emergency.',
    care: 'There is no safe home treatment to recommend here — this symptom is treated as urgent.',
    seek: 'Chest pain, especially with sweating, breathlessness, or pain spreading to the arm/jaw, needs emergency care immediately.'
  },
  'Dizziness': {
    desc: 'Often linked to dehydration, low blood sugar, inner-ear issues, or standing up too fast.',
    care: 'Sit or lie down, sip water, and rise slowly. Eating something if you haven\u2019t recently can help.',
    seek: 'Dizziness with fainting, slurred speech, or weakness on one side needs urgent evaluation.'
  },
  'Rash': {
    desc: 'Commonly irritation, allergy, heat, or a viral illness; presentation varies widely.',
    care: 'Keep the area clean and dry, avoid known irritants, and try an OTC antihistamine or hydrocortisone cream for itching.',
    seek: 'Rash with fever, spreading rapidly, blistering, or facial/throat swelling needs urgent care.'
  },
  'Joint pain': {
    desc: 'Often from strain, overuse, or mild inflammation; can also reflect arthritis.',
    care: 'Rest, ice for swelling, gentle movement, and ibuprofen or paracetamol for pain relief.',
    seek: 'A hot, swollen joint with fever, or pain after an injury with inability to bear weight, needs prompt review.'
  },
  'Abdominal pain': {
    desc: 'Can come from digestion, gas, a stomach bug, or in some cases something needing urgent attention (e.g. appendicitis).',
    care: 'Light, bland food, fluids, and rest. A heating pad can ease cramping-type pain.',
    seek: 'Severe or worsening pain, especially in the lower right abdomen, with fever or vomiting, needs urgent assessment.'
  }
};
...
function runSymptomCheck(){
  ...
  const cards = [...selected].map(s=>{
    const info = symptomInfo[s] || { desc:'No general information available for this symptom yet.', care:'Speak with a pharmacist or clinician for guidance.', seek:'If you are concerned, contact a clinician.' };
    return `
      <div class="card" style="margin-top:14px;">
        <h3>${s}</h3>
        <p><b>What it usually is:</b> ${info.desc}</p>
        <p><b>What to try:</b> ${info.care}</p>
        <p style="margin:0;"><b>Seek care if:</b> ${info.seek}</p>
      </div>`;
  }).join('');
  ...
  box.innerHTML = `
    ...
    <p class="lede" style="margin:0 0 4px;">${hasUrgent ? 'Chest pain or breathing difficulty can indicate a serious condition. Contact emergency services or go to the nearest emergency department now.' : 'This is general guidance, not a diagnosis or a personal prescription. If symptoms worsen, persist, or you feel concerned, contact a clinician or pharmacist.'}</p>
    ${cards}
  `;
